...
class PFPascalDataset(CorrespondenceDataset):
    ...
    def __init__(self, benchmark, datapath, thres, device, 
                 split, augmentation, feature_size, aug_mode, 
                 aug_aff_scaling, aug_tps_scaling, aug_photo_weak, 
                 aug_photo_strong, aug_photo_source=0.2, additional_weak=False):
        ...
        super(PFPascalDataset, self).__init__(benchmark, datapath, thres, device, split, 
                                              ...
                                              aug_tps_scaling, aug_photo_weak, aug_photo_strong, 
                                              aug_photo_source=aug_photo_source, additional_weak=additional_weak)
        ...
        self.use = np.zeros(self.train_data.shape[0])
        rng = random.Random(0)
        lis = rng.sample(list(np.arange(self.train_data.shape[0])), int(self.train_data.shape[0]* 0.1) )
        for i in lis :
            self.use[i] = 1
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'trn' and self.augmentation:
            ...
            batch['trg_img'] = torch.cat((batch['trg_img_weak'], batch['trg_img_strong'], batch['trg_additional_weak']), dim=0)
            batch['trg_img'], batch['trg_kps'] = random_crop(batch['trg_img'], batch['trg_kps'], self.trg_bbox[idx].clone().int(), size=(self.imside,)*2)
            batch['trg_img_weak'], batch['trg_img_strong'], batch['trg_additional_weak'] = torch.chunk(batch['trg_img'], 3, dim=0)
    ...
    def horizontal_flip(self, batch):
        ...
        batch['trg_bbox'][0] = batch['trg_img_weak'].size(2) - batch['trg_bbox'][2]
        batch['trg_bbox'][2] = batch['trg_img_weak'].size(2) - tmp

        batch['src_kps'][0][:batch['n_pts']] = batch['src_img'].size(2) - batch['src_kps'][0][:batch['n_pts']]
        batch['trg_kps'][0][:batch['n_pts']] = batch['trg_img_weak'].size(2) - batch['trg_kps'][0][:batch['n_pts']]

        batch['src_img'] = torch.flip(batch['src_img'], dims=(2,))
        batch['trg_img_weak'] = torch.flip(batch['trg_img_weak'], dims=(2,))
        batch['trg_img_strong'] = torch.flip(batch['trg_img_strong'], dims=(2,))
        batch['trg_additional_weak'] = torch.flip(batch['trg_additional_weak'], dims=(2,))
